  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <button className="px-3 py-1 bg-green-600 text-white rounded">Post Job</button>
      </div>
      ...
      <div className="mt-4 space-y-2">
        {jobs.map((j) => (
          <div key={j.id} className="p-3 bg-white border rounded flex items-center justify-between">
            ...
            <div>
              <Link to={`/employer/job/${j.id}/applicants`} className="text-sm text-indigo-600">View Applicants</Link>
            </div>
          </div>
        ))}
      </div>
